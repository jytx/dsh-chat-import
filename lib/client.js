...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function DiscoveryPanel({ onClose, embedded }) {
      ...
      // 按工作区文件夹（project）分组（跨页全量，来自工作区筛选后的缓冲）：组按组内
      // 最新会话的最后编辑时间降序（最近活跃的工作区置顶，时间并列按工作区名升序稳
      // 定），组内按最后编辑时间降序；未分组钉最后。
      ...
      const groupedList = [...byProject.keys()].sort((a, b) => {
      ...
      }).map((name) => ({ name, list: [...byProject.get(name)].sort(byTimeDesc) }));

      // 整组切页：一个工作区永不跨页——平铺按时间切页会把一个项目的会话拆散到
      // 多页（实测 469 条库、50/页时某项目 31 条在首页只见 2 条），组头与展开行
      // 都会被误读为「该项目只有这几条」。按组累加填充当前页：累计将超 pageSize
      // 即换页；单组超 pageSize 时独占一页完整显示（页大小是软上限，不是硬切）。
      const groupPages = [];
      let filling = [];
      let fillingCount = 0;
      for (const g of groupedList) {
        if (filling.length > 0 && fillingCount + g.list.length > pageSize) {
          groupPages.push(filling);
          filling = [];
          fillingCount = 0;
        }
        filling.push(g);
        fillingCount += g.list.length;
      }
      if (filling.length > 0) groupPages.push(filling);
      const totalPages = Math.max(1, groupPages.length);
      // 数据变化（重扫/筛选）后 page 可能越界：夹回最后一页，避免渲染空页
      const safePage = Math.min(page, totalPages - 1);
      const groups = groupPages[safePage] || [];
      // 当前页会话 = 各组列表摊平（翻页零重扫，切片仍在客户端缓冲上）
      const sessions = groups.flatMap((g) => g.list);
      ...
      const body = React.createElement(React.Fragment, null,
          ...
          items.length > 0 && React.createElement("div", { style: style.pageBar },
            ...
            React.createElement("span", { style: style.pageInfo },
              narrow ? (safePage + 1) + "/" + totalPages : t("pagination", { page: safePage + 1, pages: totalPages, total: displayTotal })),
            React.createElement("button", {
              style: narrow ? style.iconBtn : style.pageBtn,
              disabled: safePage >= totalPages - 1 || importing,
